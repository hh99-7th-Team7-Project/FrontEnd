...
const apis = {
  ...
  getCoffeeCategory: (category) =>
    api.get(`/coffee/sidebar?category=${category}`),
  addCoffee: (brand, coffee) => instance.post(`/coffee/${brand}`, coffee),
  getRandomCoffee: () =>
    api.get('/coffee/random?brand=스타벅스&category=coffee'),

  postComment: (brand, id, data) =>
    api.post(`coffee/${brand}/${id}/review`, data),
  getComment: (brand, boardId) => api.get(`coffee/${brand}/${boardId}/review`),
  deleteComment: (brand, boardId, reviewId) =>
    api.delete(`/coffee/${brand}/${boardId}/review/${reviewId}`, reviewId),
  updatecomment: (brand, boardId, reviewId, data) =>
    api.put(`/coffee/${brand}/${boardId}/review/${reviewId}`, data),

  prechat: () => instance.get(`/mainchat/get/main`),
  prepostchat: (pid) => api.get(`/postchat/get/${pid}`),
};
